...
export const ApiReferenceContent = ({ functions, host }: Props) => (
  <div className="space-y-10">
    ...
    <Section id="quickstart" title="Quickstart">
      <Prose>
        Send the document in the <code className="font-mono text-xs">file</code> field and any options as a JSON string
        in <code className="font-mono text-xs">args</code>. Exactly one file per request.
      </Prose>
    ...
    <Section id="errors" title="Errors">
      <Prose>
        Failures use one envelope. <code className="font-mono text-xs">retryable</code> says whether repeating the same
        request could succeed — a 429 will, a 400 will not.
      </Prose>
    ...
    <Section id="rate-limits" title="Rate limits">
      <Prose>
        ...
        returns <code className="font-mono text-xs">RATE_LIMITED</code>, which is retryable — back off and retry rather
        than looping. Separately, your plan caps documents per billing period; exhausting that returns{" "}
    ...
    <Section id="webhooks" title="Webhooks">
      <Prose>
        ...
        <code className="font-mono text-xs">X-Heirs-Delivery</code> (stable across retries — use it to make your handler
        idempotent) and <code className="font-mono text-xs">X-Heirs-Event</code>.
      </Prose>
      <Prose>
