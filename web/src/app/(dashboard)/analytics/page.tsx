...
import { PageLayout, StatTile } from "@/components/shared";
  ...
  return (
    <PageLayout title="Analytics" subtitle="Request volume, errors, and token usage across the service.">
      <div className=" space-y-6">
        {/* Summary tiles */}
        {metrics.isPending && <p className="text-sm text-muted-foreground">Loading metrics…</p>}
        {metrics.isError && (
        ...
        <section className="space-y-2">
          ...
          {usage.isPending && <p className="text-sm text-muted-foreground">Loading usage…</p>}
          {usage.isError && (
            <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">
