...
`web/` is a **Next.js** app (its own pnpm project; `output: "standalone"`) serving both operator
surfaces from one deployment:

- **Admin area** (`/analytics`, `/tenants`, `/users`, `/subscriptions`, `/system-health`, …) —
  needs an `admin_session` cookie; login at `/admin/login`.
- **Tenant area** (`/ocr`, `/keys`, `/team`) — needs a `tenant_session` cookie; login at `/login`.

`web/src/proxy.ts` (Next 16 Proxy, formerly Middleware) is an **optimistic** role-aware gate —
it only checks cookie presence; real validation happens in the backend on every proxied call.
Server-side API routes under `web/src/app/api/*` proxy to the backend (`OCR_API_URL`), attaching
the API key server-side so it never reaches the browser (`web/.env.local.example`). Stack:
React Query, React Hook Form + Zod, Base UI, Tailwind.
